...
(function() {
  var apply_force, apply_forces, apply_friction, chaser_attraction, detect_end, detect_hits, ep, get_pairs, get_sprite_group, get_time, get_types, hit, init_mobs, init_obj, mob_repulsion, move_dante, move_sprites, runner_repulsion, size, zero_forces,
  ...
  init_mobs = function(go, n, m, t) {
    ...
    for (i = j = 1, ref = n; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
      ...
      results.push(go.sprites[t + '_' + i] = init_obj(cx, cy, r, c, t));
    }
    return results;
  };

  detect_hits = function(go) {
    var d, name, p, ref, results, sprite, sprites, x1, x2, y1, y2;
    sprites = go.sprites;
    p = go.sprites.dante;
    results = [];
    for (name in sprites) {
      ...
      if (d < (sprite.r / 2 + p.r / 2) && go.hit_ctr <= 0 && name !== 'dante') {
  ...
  init_obj = function(cx, cy, r, img, type) {
    return {
      cx: cx,
      cy: cy,
      r: r,
